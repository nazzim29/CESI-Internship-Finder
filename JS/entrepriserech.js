...
document.getElementById('sr').addEventListener('keyup',function(){
  showresult(document.getElementById('sr').value);
});
function getall() {
    var xhr = new XMLHttpRequest();
    xhr.responseType = 'json';
...
function showresult(saha = null) {
  ...
  for(var i = (current_page-1)*8;i<current_page*8;i++){
    ...
    card+= filtred_user[i].Id_entreprise +".png\" alt=\"logo\" class=\"img-fluid rounded-circle w-50\"></div><ul class=\"info\"><li>Raison sociale: ";
    card+= filtred_user[i].Raison_social +"</li><li>Secteur d'activité: ";
